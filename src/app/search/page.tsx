  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100 font-sans">
      ...
      <main className="flex-1 mx-auto max-w-7xl w-full px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col md:flex-row items-start justify-between border-b border-slate-800 pb-6 mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-white">Find Local Service Pros</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          {/* Filters Sidebar */}
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 h-fit space-y-6">
            <div className="flex items-center space-x-2 border-b border-slate-800 pb-3">
              <Filter className="h-4 w-4 text-blue-400" />
              <h2 className="font-bold text-white text-sm">Filter Results</h2>
            </div>
              ...
              <div className="space-y-1">
                <Link
                  ...
                  className={`block px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                    !categorySlug ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
                  }`}
                >
                  All Categories
                </Link>
                ...
                {[0, 4.0, 4.5, 4.8].map((rat) => (
                  <Link
                    ...
                    className={`flex items-center justify-between px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                      minRating === rat ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800'
                    }`}
                  >
                    <span>{rat === 0 ? 'Any Rating' : `${rat}★ & higher`}</span>
                    {rat > 0 && <Star className="h-3 w-3 fill-amber-400 text-amber-400" />}
                  </Link>
                ))}
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-slate-800">
              ...
              >
                ...
                {verifiedOnly && <CheckCircle2 className="h-4 w-4 text-emerald-400" />}
              </Link>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            {businesses.length === 0 ? (
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center">
                ...
                <Link
                  href="/search"
                  className="mt-4 inline-block bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium px-4 py-2 rounded-xl transition"
                >
                  Reset Filters
                </Link>
              </div>
              ...
              businesses.map((biz: any) => (
                <div
                  key={biz.id}
                  className="bg-slate-900 border border-slate-800 hover:border-slate-700 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 transition shadow-md hover:shadow-xl"
                >
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        {biz.verification_status === 'approved' && (
                          <span className="bg-emerald-500/20 text-emerald-300 text-[10px] font-semibold px-2 py-0.5 rounded-full border border-emerald-500/30 flex items-center space-x-1">
                      ...
                      <p className="text-xs text-slate-400 mt-1 line-clamp-2">{biz.description}</p>
                  ...
                  <div className="w-full sm:w-auto flex sm:flex-col items-center sm:items-end justify-between border-t sm:border-t-0 pt-4 sm:pt-0 border-slate-800">
                    <Link
