    ...
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100 font-sans">
      ...
      <main className="flex-1 mx-auto max-w-7xl w-full px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800 pb-6 mb-8 gap-4">
          <div>
            <span className="text-xs font-semibold text-blue-400 uppercase tracking-wider">Customer Portal</span>
                ...
                >
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-slate-400">
                      Provider:{' '}
                      <span className="text-white font-medium">{booking.businesses?.name}</span>
                    </p>
                    ...
                    <div className="flex items-center space-x-2">
                      {booking.status === 'completed' && (
                        <button
                          ...
                          className="bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 text-xs font-semibold px-3.5 py-1.5 rounded-xl transition flex items-center space-x-1"
                        >
                      ...
                      {(booking.status === 'pending' || booking.status === 'accepted' || booking.status === 'confirmed') && (
                        <button
                          ...
                          className="bg-rose-500/20 hover:bg-rose-500/30 border border-rose-500/40 text-rose-300 text-xs font-semibold px-3.5 py-1.5 rounded-xl transition flex items-center space-x-1"
                        >
          ...
          <div className="space-y-3 max-w-3xl">
            {notifications.length === 0 ? (
              ...
              notifications.map((notif) => (
                <div
                  key={notif.id}
                  className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-start space-x-3"
                >
        ...
        {activeTab === 'favorites' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {favorites.length === 0 ? (
              ...
              favorites.map((fav) => (
                <div key={fav.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center space-x-4">
                  <img
                  ...
                  />
                  <div>
                    <h4 className="text-sm font-bold text-white">{fav.businesses?.name}</h4>
                    <p className="text-xs text-slate-400">{fav.businesses?.city}</p>
                  </div>
                </div>
              ))
            )}
        ...
        {cancelBookingId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            <form onSubmit={handleCancelBookingSubmit} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4">
              ...
              <div>
                <label className="block text-xs text-slate-300 mb-1">Reason for Cancellation (Optional)</label>
                <textarea
                  rows={3}
                  value={cancelReason}
        ...
        {reviewBookingId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            <form onSubmit={handleReviewSubmit} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4">
                  ...
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      ...
                      className={`p-2 rounded-lg border ${
                        reviewRating >= star ? 'bg-amber-500/20 border-amber-500/40 text-amber-400' : 'border-slate-800 text-slate-600'
                      }`}
                    >
