        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-800 pb-6">
            <div className="flex items-start space-x-5">
              <img
                ...
                alt={business.name}
                className="h-24 w-24 rounded-2xl object-cover border-2 border-slate-800 shrink-0 bg-slate-950"
              />

              <div>
                <div className="flex items-center space-x-3">
                  <h1 className="text-2xl sm:text-3xl font-extrabold text-white">{business.name}</h1>
                  {business.verification_status === 'approved' && (
                    <span className="bg-emerald-500/20 text-emerald-300 text-xs font-semibold px-3 py-1 rounded-full border border-emerald-500/30 flex items-center space-x-1">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {business.phone && (
                <a
                  ...
                  className="bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold px-4 py-2.5 rounded-xl transition flex items-center space-x-2"
                >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-4">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div>
                <h2 className="text-xl font-bold text-white mb-2">About the Business</h2>
              ...
              <div>
                <h2 className="text-xl font-bold text-white mb-4">Available Services & Instant Booking</h2>
                <div className="space-y-4">
                  {services?.map((service: any) => (
                    <div
                      key={service.id}
                      className="bg-slate-950 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
                    >
                      <div>
                      ...
                      <div className="w-full sm:w-auto flex items-center justify-between sm:justify-end space-x-4 border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-800">
                      ...
                      <div key={rev.id} className="bg-slate-950 border border-slate-800 rounded-2xl p-4 space-y-2">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            ...
                            />
                            <span className="text-sm font-semibold text-white">{rev.profiles?.full_name || 'Anonymous'}</span>
                          </div>
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5">
                ...
                <div className="space-y-2 text-xs">
                  {daysOfWeek.map((dayName, idx) => {
                    ...
                    return (
                      <div key={dayName} className="flex items-center justify-between text-slate-300 py-1 border-b border-slate-900">
                        <span>{dayName}</span>
                        {dayObj && !dayObj.is_closed ? (
                          <span className="text-slate-400">{dayObj.open_time} - {dayObj.close_time}</span>
                        ) : (
