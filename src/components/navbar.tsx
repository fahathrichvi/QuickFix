    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-800 bg-slate-950/80 backdrop-blur-md text-slate-100">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8 h-16">
        ...
        <nav className="hidden md:flex items-center space-x-8 text-sm font-medium">
          <Link href="/search" className="flex items-center space-x-1.5 text-slate-300 hover:text-white transition-colors">
            <Search className="h-4 w-4 text-blue-400" />
            <span>Find Services</span>
          </Link>
          <Link href="/search?category=plumbing-drainage" className="text-slate-300 hover:text-white transition-colors">
            Plumbing
          </Link>
          <Link href="/search?category=electrical-wiring" className="text-slate-300 hover:text-white transition-colors">
            Electrical
          </Link>
          <Link href="/search?category=hvac-air-conditioning" className="text-slate-300 hover:text-white transition-colors">
            HVAC
          </Link>
        </nav>
          ...
          {user ? (
            <div className="flex items-center space-x-3">
              <Link
                ...
                className="flex items-center space-x-2 rounded-lg bg-slate-900 border border-slate-800 px-3.5 py-2 text-sm font-medium text-slate-200 hover:bg-slate-800 hover:text-white transition"
              >
          ...
          {user ? (
            <div className="pt-2 border-t border-slate-800 space-y-2">
              <Link
                href={getDashboardHref()}
                onClick={() => setIsMobileMenuOpen(false)}
                className="block text-blue-400 font-medium py-1"
              >
            ...
            <div className="pt-2 border-t border-slate-800 space-y-2">
              ...
              <Link
                href="/auth/register"
                onClick={() => setIsMobileMenuOpen(false)}
                className="block font-medium text-blue-400 py-1"
              >
                Create Account
              </Link>
            </div>
